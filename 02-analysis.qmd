...
## Average Water Boil Notice Length 
...
mapview(water_length_shape, zcol= "average_days", col.regions= c("#5ae2f5", "#83cbf6", "#5a9ef5", "blue", "#0c1355")) 
```
...
mapview(water_length_shape, zcol= "max_days", col.regions= c("#a3eab3","#65e382", "#3ae360", "#1bb83f","#014e12")) 
```


**Data Takeaways**: 

- The counties with the longest lasting water boil notices are more rural (Polk and Wood). This is different from the longest average results. Rural counties have smaller populations and lower demand for water boil notices to be resolved quickly. 

## Minimum length of water boil notices 

Lastly, let's look at the minimum amount of days that water boil notices last in Texas. 

```{r}
#| warning: false

mapview(water_length_shape, zcol= "min_days", col.regions= c("#eacef9","#c56ef3", "#7313a5", "#3d045b")) 

```

**Data Takeaways**: 

- Here we can see that when it comes to the minimum number of days that it takes to resolve a water boil notice, rural counties like Presidio or counties near the border or Panhandle have longer wait times. 
- There is also a pattern in heavily populated areas (like the Houston metro area) where water boil notices can be resolved in just one day. 
...
- The counties with the most ongoing water notices are Harris, Johnson and Wise. Again, Harris county has a very high amount of water boil notices. 
...
- In the state of Texas, there were at least 69 total ongoing water boil notices (in all counties and regions) at the time that this data was collected. 

## Harris County notices  
...
population_shape <- counties |>
  left_join(population_ongoing_notice, by= c("NAME"= "county"))


```

I followed the same code as when looking at water boil notice duration. Then I joined the analysis with the shape file. 

```{r}
#| warning: false

mapview(population_shape, zcol= "county_population_notice", col.regions= c("#febfbf","#fd6f6f", "#ff2e2e", "#780404")) 
```
...
Let's try this by region and look at populations under water boil notices: 
...
- Region 12 which covers Harris county surpasses all other counties in terms of population affected. 
...
- When this data was collected, at least 56,074 Texas were affected by water boil notices. 

Now let's look at the population affected each year. I grouped by year both issued and rescinded. NA represents ongoing notices. 

```{r}
#| warning: false

water_boil |>
  group_by(year(issued), year(rescinded)) |>
  summarise(sum(population)) 
```

**Data Takeaways**: 

- There is no clear pattern to the volume of water boil notices. 

```{r}
water_boil |>
  summarise(total_affected = sum(population))
```

**Data Takeaway**: 

- At least 57,049,113 people were affected by water boil notices between 2013 and 2024. 
...
- Water boil notices are more related to system wide issues, rather than localized. But, not by that much of a difference.  
...
**ONGOING NOTICES AND AFFECTED POPULATION**: At the time this data was collected at least 56,074 people in the state of Texas were affected by water boil notices. In total, water boil notices affected more than 57 million people in the state of Texas in more than a decade. 
